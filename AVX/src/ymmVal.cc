...
#define IMPLE_CHAR_FUN(x) T##x(char* buf, size_t sz, bool isHighPart)
...
char* YmmVal::IMPLE_CHAR_FUN(oString_i64) {
	if (!isHighPart) {
		snprintf(buf, sz, "%16lld %16lld", i64[0], i64[1]);
		buf[strlen(buf)] = '\0';
	}
	else {
		snprintf(buf, sz, "%16lld %16lld", i64[2], i64[3]);
		buf[strlen(buf)] = '\0';
	}	
...
char* YmmVal::IMPLE_CHAR_FUN(oString_u64) {
	if (!isHighPart) {
		snprintf(buf, sz, "%16llu %16llu", u64[0], u64[1]);
		buf[strlen(buf)] = '\0';
	}
	else {
		snprintf(buf, sz, "%16llu %16llu", u64[2], u64[3]);
		buf[strlen(buf)] = '\0';
	}	
...
char* YmmVal::IMPLE_CHAR_FUN(oString_x32) {
	if (!isHighPart) {
		snprintf(buf, sz, "%08x %08x %08x %08x",
			u32[0], u32[1], u32[2], u32[3]);
		buf[strlen(buf)] = '\0';
	}
	else {
...
char* YmmVal::IMPLE_CHAR_FUN(oString_x64) {
	if (!isHighPart) {
		...
		snprintf(buf, sz, "%016llx %016llx", u64[2], u64[3]);
		buf[strlen(buf)] = '\0';
	}	
...
char* YmmVal::IMPLE_CHAR_FUN(oString_r64) {
	if (!isHighPart) {
		...
		snprintf(buf, sz, "%.12lf %.12lf",r64[2], r64[3]);
		buf[strlen(buf)] = '\0';
	}
